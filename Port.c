...
static volatile uint32 *getPortBasePtr(Port_PinType PortPinId)
{
  ...
  uint8 PortId = PortPinId / NUM_OF_PINS_IN_SINGLE_PORT;
...
{
  return PortPinId / NUM_OF_PINS_IN_SINGLE_PORT;
}
...
void Port_Init(const Port_ConfigType *ConfigPtr)
{
  ...
  for (Port_Pin_Index; Port_Pin_Index < PORT_NUMBER_OF_CONFIGERED_PINS; Port_Pin_Index++)
  {
...
#endif /* (PORT_DEV_ERROR_DETECT == STD_ON) */
  {
    for (Port_PinType Pin = 0; Pin < PORT_NUMBER_OF_CONFIGERED_PINS; Pin++)
    {
      /* if the configuration ptr is Null, head On to next Port-Pin Index */
      if (Port_pinConfigurationSet.Pins[Pin] == NULL_PTR)
      {
